...
3. **Verify the outer gate** ran + green. By default that is `pnpm run typecheck` (repo-wide) AND the scoped suite ``pnpm vitest run <path-or-pattern>` (app), `pnpm --filter vinta-schedule-design-system test` (design system)` covering the touched apps. {If run_options.full_test_suite = true:} the outer gate runs `pnpm run typecheck` AND the full `pnpm run test` instead — verify that. Look in the report for explicit confirmation the applicable gate was executed + passed + the e2e suite when `run_options.run_e2e = true`. Vague confirmation → **re-run yourself** (in `<WORKROOT>`).
...
6b. **Dependency license scan**: `git -C <WORKROOT> diff <BASE_BRANCH>..HEAD -- package.json packages/design-system/package.json pnpm-lock.yaml` — for every added dep look up its SPDX license (`npm view <pkg> license`). A license in `policies.dependency_licenses.forbidden_spdx` (`GPL-2.0-only`, `GPL-3.0-only`, `AGPL-3.0-only`, `SSPL-1.0`) and not in `allowed_overrides` is a **BLOCKER** (enforcement is `block`). A missing / `UNKNOWN` / undeclared license is **always a BLOCKER** — there is no override to silently bless undisclosed terms.
7. **AI co-author scan**: `git -C <WORKROOT> log <BASE_BRANCH>..HEAD --format='%B'` — grep for `Co-Authored-By`, `Co-authored-by`, `Generated with`, `Claude`, `noreply@anthropic.com`. Any hit is a **BLOCKER**: this repo forbids AI co-author trailers. The fix is a message rewrite (`git commit --amend` / `git rebase -i`) before the branch is pushed.
...
5b. **E2E spec + screenshots** — only when `run_options.run_e2e = true` (otherwise a no-op): the phase's named spec exists at `e2e/tests/app/<ID>-<slug>.spec.ts`, its `<ID>` matches a row in [QA_USE_CASES.md](QA_USE_CASES.md) with the spec path filled in, and the screenshots it captures landed in `pr-screenshots/`.
...
After Layers 1–2 pass, spawn a **separate** subagent (different session, no implementation context) using the project's `reviewer` agent type ([ai-tools/agents/reviewer.yaml](ai-tools/agents/reviewer.yaml)) at the model resolved from `agent_models.reviewer` (see the [Resolve the reviewer + fixer model](#resolve-the-reviewer--fixer-model) step; unset → runtime default). Read-only by design.
...
1. Spawn a **new** subagent — the project's `fixer` agent type ([ai-tools/agents/fixer.yaml](ai-tools/agents/fixer.yaml)) at the model resolved from `agent_models.fixer` (see the [Resolve the reviewer + fixer model](#resolve-the-reviewer--fixer-model) step; unset → runtime default). The fix prompt quotes the finding verbatim. For comment-hygiene findings (Layer 2 item 8), the fix prompt tells the fixer to run the `deslop-comments` skill ([ai-tools/skills/deslop-comments/SKILL.md](ai-tools/skills/deslop-comments/SKILL.md)) scoped to the phase's touched files — comment-only edits, no behavior change.
