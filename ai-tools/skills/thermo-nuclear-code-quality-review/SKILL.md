...
This skill is read-only, like the project's normal review. It does not edit code. When a finding warrants a change, hand it to the project's `fixer` agent ([ai-tools/agents/fixer.yaml](ai-tools/agents/fixer.yaml)) with the finding quoted verbatim, exactly as the standard fix loop does.

The per-phase review (the `reviewer` agent, [ai-tools/agents/reviewer.yaml](ai-tools/agents/reviewer.yaml)) applies a condensed version of standard 0 on every phase — "is there an obvious code-judo reframe?" When that lens surfaces something structural that deserves a full audit, the reviewer escalates by invoking this skill against the phase diff. Reserve the full pass for when it earns its cost; the condensed lens is enough for routine phases.
