...
- `PR creation policy: **agents create PRs** — after a phase passes review, the orchestrator pushes the branch and opens the PR itself via the prs-context file + the bundled `open-pr.sh` (which shells out to `gh`).` policy + `run_options.generate_inline_comments`.
...
| `PR creation policy: **agents create PRs** — after a phase passes review, the orchestrator pushes the branch and opens the PR itself via the prs-context file + the bundled `open-pr.sh` (which shells out to `gh`).` policy | `run_options.generate_inline_comments` | What this step does |
|---|---|---|
