...
/**
 * deleteCalendarGroup — best-effort cleanup for a calendar group this
 * suite's specs created in the shared QA org.
 *
 * There is no delete affordance anywhere in the app UI for a calendar group
 * (`groups-table.tsx` and `group-detail-view.tsx` only offer "Get scheduling
 * link" and "Edit") — `calendarGroupsDestroy` (`DELETE /calendar-groups/{id}/`)
 ...
 */
export async function deleteCalendarGroup(
  ...
  if (!apiBaseUrl || !accessToken) {
    console.warn(
      `[cleanup] Skipping delete of calendar group ${groupId} — ` +
  ...
  try {
    ...
    const response = await request.delete(
      `${apiBaseUrl}/calendar-groups/${groupId}/`,
      {
        headers: {
    ...
    if (!response.ok() && response.status() !== 404) {
      console.warn(
        `[cleanup] Failed to delete calendar group ${groupId}: ` +
          `${response.status()} ${await response.text()}`
      );
    }
  } catch (err) {
    console.warn(
      `[cleanup] Error deleting calendar group ${groupId}: ${String(err)}`
    );
  }
}
