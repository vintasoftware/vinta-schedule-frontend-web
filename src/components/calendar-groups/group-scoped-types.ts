/**
 * group-scoped-types — pure, framework-free view-model for the weekday
 ...
 * -----------------------------------------------------------------------
 * A `GroupScopedAvailabilityWindow` is representable in the weekday grid
...
export interface WeekdayWindow {
   ...
   * `GroupScopedAvailabilityWindow`; absent for a row the editor added and
...
export type ClassifiedWindow =
  ...
  | { kind: 'unrepresentable'; window: GroupScopedAvailabilityWindow };
...
/**
 * Classifies one `GroupScopedAvailabilityWindow` as either a grid row
...
export interface ClassifiedWindows {
  ...
  unrepresentable: GroupScopedAvailabilityWindow[];
}
...
export interface GridDiff<T extends WeekdayWindow = WeekdayWindow> {
   ...
   * group-scoped-types.test.ts's "unrepresentable rows can never enter
 ...
 * (i.e. whatever this calendar's group-scoped rows are already configured
 * with), falling back to the viewer's own timezone when the calendar has no
 * group-scoped windows at all yet. `Calendar` carries no timezone field of
