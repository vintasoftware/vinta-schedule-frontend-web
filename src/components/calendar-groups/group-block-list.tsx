...
/**
 * GroupBlockList — the group-scoped blocked-time rows for one calendar in
 ...
 * individually through GroupBlockForm, mounted inside a Dialog this
 * component owns.
 *
 * Owns the alert state that group-window-grid.tsx owns for windows, but
 ...
 * orphans" or "rejected" (handled inline by GroupBlockForm itself -- see
 ...
 * structurally identical between `GroupScopedAvailabilityOrphanedBooking`
 * and `GroupScopedBlockOrphanedBooking`, see that component's doc comment.
 */
...
import { GroupBlockForm } from './group-block-form';
import {
...
interface BlockRowProps {
  ...
  readOnly: boolean;
  isDeleting: boolean;
  onEdit: (block: GroupScopedBlockedTime) => void;
...
function BlockRow({
  ...
  return (
    <HStack
      ...
      border
      radius='md'
      data-testid={`group-block-${block.id}`}
    >
      <Stack gap={1}>
...
export interface GroupBlockListProps {
  groupId: number;
  slotId: number;
  calendarId: number;
...
export function GroupBlockList({
  groupId,
  slotId,
  calendarId,
  calendarName,
  ...
  const { blocks, isLoading, isTruncated, deleteBlock } = useGroupScopedBlocks({
    groupId,
    slotId,
    calendarId,
  });
  ...
  const [editingBlock, setEditingBlock] =
    React.useState<GroupScopedBlockedTime | null>(null);
  ...
  const openEdit = (block: GroupScopedBlockedTime) => {
  ...
  const handleDelete = React.useCallback(
    async (id: number) => {
      ...
      try {
        const result = await deleteBlock({ groupId, slotId, blockId: id });
  ...
  return (
    <VStack gap={3} data-testid='group-block-list'>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            ...
            {/* Conditionally mounted (not just visually hidden) so switching
                ...
                GroupBlockForm a fresh mount -- see that file's doc comment
