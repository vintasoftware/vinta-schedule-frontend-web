...
function makeCreateResponse(block: GroupScopedBlockedTime) {
  return {
...
function makeUpdateResponse(block: GroupScopedBlockedTime) {
  return {
    data: { block, orphaned_bookings: [] },
    response: new Response(null, { status: 200 }),
  } as unknown as Awaited<
    ReturnType<typeof calendarGroupsSlotsBlockedTimesPartialUpdate>
  >;
}
...
function renderForm(
  props: Partial<React.ComponentProps<typeof GroupBlockForm>> = {}
) {
  ...
  return render(
    <GroupBlockForm groupId={1} slotId={10} calendarId={42} {...props} />,
    { wrapper }
  );
}
...
describe('GroupBlockForm', () => {
  ...
  describe('create', () => {
    it('a one-off block submits without an rrule', async () => {
      ...
      // tests below don't drive it either. group-block-list.test.tsx and
      ...
      await waitFor(() =>
        expect(calendarGroupsSlotsBlockedTimesCreate).toHaveBeenCalledTimes(1)
      );

      const call = vi.mocked(calendarGroupsSlotsBlockedTimesCreate).mock
        .calls[0]?.[0] as { body: GroupScopedBlockedTimeCreate };
    ...
    it('start_time >= end_time is rejected by the form before any request', async () => {
      ...
      expect(calendarGroupsSlotsBlockedTimesCreate).not.toHaveBeenCalled();
    });
    ...
    it('a 402 over-limit rejection renders the shared alert inline and keeps the input on screen', async () => {
      ...
      vi.mocked(calendarGroupsSlotsBlockedTimesCreate).mockImplementation(
        (async () => {
          throw overLimitBody;
        }) as unknown as typeof calendarGroupsSlotsBlockedTimesCreate
      );
