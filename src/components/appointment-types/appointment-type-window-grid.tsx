...
/**
 * AppointmentTypeWindowGrid — the weekday grid editor for one calendar's appointment-type-scoped
 ...
 * (appointment-type-scoped-types.ts) compares the edited rows against the rows the
 * grid was loaded with and issues only the resulting creates, updates, and
 * deletes through Phase 3a's `useAppointmentTypeScopedWindows` hook. An unchanged
 ...
 * this component's save handler (see appointment-type-scoped-types.test.ts).
 *
 * Timezone: a single grid-level selector, defaulting to the calendar's own
 * already-configured timezone (read from its existing appointment-type-scoped rows --
...
export interface AppointmentTypeWindowGridProps {
...
// Extra bookkeeping AppointmentTypeWindowGrid attaches to each edited row so a
...
// shape, the same convention useAppointmentTypeScopedWindows.deleteWindow already
...
export function AppointmentTypeWindowGrid({
  appointmentTypeId,
  slotId,
  calendarId,
  calendarName,
}: AppointmentTypeWindowGridProps) {
  // Read-only-ness comes from the shared AppointmentTypePermissionsProvider context
  // (mounted by the appointment type detail page), the same predicate every roster row
  // and editor in this feature consumes -- see appointment-type-permissions.ts.
        ...
        try {
          const outcomes = await Promise.allSettled<WriteOutcome>([
            ...
            ...diff.deletes.map(async (id): Promise<WriteOutcome> => {
              await deleteWindow({ appointmentTypeId, slotId, windowId: id });
