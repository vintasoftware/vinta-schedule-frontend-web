/**
 * AppointmentTypeWindowGrid tests.
 *
 * Covers:
 ...
 *   regression -- see appointment-type-scoped-types.ts's classifyWindow doc and the
 *   phase-3b review);
 ...
 *   component level (appointment-type-scoped-types.test.ts proves it at the pure-
 *   function level).
 *
 * Phase 3c additions:
...
describe('AppointmentTypeWindowGrid', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('ticking two weekdays and saving issues exactly two creates with single-BYDAY weekly rrules', async () => {
    ...
    vi.mocked(
      ...
      )) as unknown as typeof appointmentTypesSlotsAvailabilityWindowsCreate);
    ...
    await waitFor(() =>
      expect(
        vi.mocked(appointmentTypesSlotsAvailabilityWindowsCreate)
      ).toHaveBeenCalledTimes(2)
    );

    const rruleStrings = vi
      .mocked(appointmentTypesSlotsAvailabilityWindowsCreate)
      .mock.calls.map(
        (call) =>
    ...
    expect(
      vi.mocked(appointmentTypesSlotsAvailabilityWindowsPartialUpdate)
    ).not.toHaveBeenCalled();
    expect(
      vi.mocked(appointmentTypesSlotsAvailabilityWindowsDestroy)
    ).not.toHaveBeenCalled();
  });
  ...
  it('a double submit issues one write, not two', async () => {
    ...
    const pending = new Promise<
      Awaited<ReturnType<typeof appointmentTypesSlotsAvailabilityWindowsCreate>>
    >((resolve) => {
      resolveCreate = resolve;
    });
    vi.mocked(appointmentTypesSlotsAvailabilityWindowsCreate).mockReturnValue(
      pending as ReturnType<
        typeof appointmentTypesSlotsAvailabilityWindowsCreate
      >
    );
  ...
  it('renders one grid row and two read-only entries, and saving the grid never touches the unrepresentable rows’ ids', async () => {
    ...
    vi.mocked(
      appointmentTypesSlotsAvailabilityWindowsDestroy
    ).mockResolvedValue(makeDestroyResponse(204));
  ...
  it('a 404 on update renders the gone message, clears the stale id, and refetches', async () => {
    ...
    }) as unknown as typeof appointmentTypesSlotsAvailabilityWindowsPartialUpdate);
    vi.mocked(
      appointmentTypesSlotsAvailabilityWindowsCreate
    ).mockImplementation((async (opts: {
      body: AppointmentTypeScopedAvailabilityWindowCreate;
