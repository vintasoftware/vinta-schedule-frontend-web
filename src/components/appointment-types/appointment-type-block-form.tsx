...
/**
 * AppointmentTypeBlockForm — rhf + zod form to create or edit one appointment-type-scoped blocked
 ...
 * to the appointment-type-scoped write shape (`AppointmentTypeScopedBlockedTimeCreate` /
 * `PatchedAppointmentTypeScopedBlockedTimeUpdate` — full ISO `start_time`/`end_time`
 * plus a separate `timezone`, matching appointment-type-scoped-types.ts's
 * `buildWindowCreateBody`/`buildWindowUpdateBody` convention, not the base
 * blocked-time form's naive-local-string convention).
 *
 * No Dialog chrome here on purpose — the caller (appointment-type-block-list.tsx) owns
 ...
 *    appointment-type-block-list.tsx. This sidesteps the Radix `Select` hydration bug
 *    appointment-type-window-grid.tsx hit (a controlled value that changes AFTER mount,
 ...
 * narrowing update does) — see use-appointment-type-scoped-blocks.ts's doc comment.
 ...
 * appointment-type-window-grid.tsx's UC-6 handling. Blocked-time writes are not
...
// on `appointmentTypeBlockFormSchema.timezone`).
...
const appointmentTypeBlockFormSchema = z
  .object({
    ...
    timezone: z
      ...
      // typo'd zone -- that shipped as a bug in appointment-type-window-grid.tsx's
...
type AppointmentTypeBlockFormSchema = z.infer<
  typeof appointmentTypeBlockFormSchema
>;
...
function getDefaultValues(timezone: string): AppointmentTypeBlockFormSchema {
  return {
 ...
 * mirroring `classifyWindow` in appointment-type-scoped-types.ts for the weekday grid.
...
): AppointmentTypeBlockFormSchema {
...
function buildRecurrenceRule(
  values: AppointmentTypeBlockFormSchema
): RecurrenceRule {
 ...
 * (see appointment-type-scoped-types.ts's `requireISO`, the same fix applied there).
 */
...
interface RecurrenceFieldsProps {
  form: UseFormReturn<AppointmentTypeBlockFormSchema>;
  disabled: boolean;
  /**
          ...
          <HStack gap={2} wrap>
            {WEEKDAYS.map((day) => (
              <HStack key={day.byday} gap={1} align='center'>
                <Checkbox
                  id={`appointment-type-block-byday-${day.byday}`}
                ...
                <Label
                  htmlFor={`appointment-type-block-byday-${day.byday}`}
...
export interface AppointmentTypeBlockFormProps {
...
export function AppointmentTypeBlockForm({
  appointmentTypeId,
  slotId,
  calendarId,
  block,
  ...
  const { createBlock, updateBlock } = useAppointmentTypeScopedBlocks({
    appointmentTypeId,
    slotId,
    calendarId,
    // The list this hook would otherwise fetch is already loaded by the
    // caller (appointment-type-block-list.tsx uses the same hook, same query key, so
    // TanStack Query dedupes it) -- this form only needs the mutation
    // functions, not a second read of the list.
    enabled: false,
  });

  const form = useForm<AppointmentTypeBlockFormSchema>({
    resolver: zodResolver(appointmentTypeBlockFormSchema),
    defaultValues: block
  ...
  async function onSubmit(values: AppointmentTypeBlockFormSchema) {
    setIsSaving(true);
      ...
      <FormLayout gap={4} onSubmit={form.handleSubmit(onSubmit)} noValidate>
        <Stack gap={4}>
          ...
          <FormField
            ...
            render={({ field }) => {
              ...
              // appointment-type-window-grid.tsx's timezone field, so an already-
        ...
        <HStack gap={3} justify='end'>
          ...
          <Button
            type='submit'
            disabled={isSaving}
            data-testid='appointment-type-block-submit'
          >
            {isSaving
              ? isEdit
                ? 'Saving…'
