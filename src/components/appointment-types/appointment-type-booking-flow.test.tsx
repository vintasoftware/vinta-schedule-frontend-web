/**
 * AppointmentTypeBookingFlow tests.
 *
 ...
 *   appointmentTypesEventsCreate with the right slot → calendar assignments.
 */
...
function mockAppointmentTypesList() {
  vi.mocked(appointmentTypesList).mockResolvedValue({
    data: { count: 1, results: [APPOINTMENT_TYPE] },
    response: new Response('{}', { status: 200 }),
  } as unknown as Awaited<ReturnType<typeof appointmentTypesList>>);
}

function mockAvailability(slots: AppointmentTypeRangeAvailability['slots']) {
...
function renderFlow() {
  ...
  return render(<AppointmentTypeBookingFlow open onOpenChange={vi.fn()} />, {
    wrapper: Wrapper,
  });
}

/** Pick the "Clinic" appointment type from the Radix Select. */
...
const submitBtn = () => screen.getByTestId('appointment-type-book-submit');

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

describe('AppointmentTypeBookingFlow', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockAppointmentTypesList();
  });
  ...
  it('backend-rejection race: shows race alert, re-checks availability, no dialog close', async () => {
    ...
    vi.mocked(appointmentTypesAvailabilityCreate).mockResolvedValueOnce({
      data: {
        count: 1,
        results: [
          {
    ...
    render(<AppointmentTypeBookingFlow open onOpenChange={onOpenChange} />, {
      wrapper: RaceWrapper,
    });
    ...
    // appointmentTypesAvailabilityCreate called a second time for the refresh.
    expect(appointmentTypesAvailabilityCreate).toHaveBeenCalledTimes(2);
  ...
  it('success path: complete satisfiable selection books with correct slot assignments', async () => {
    ...
    await waitFor(() =>
      expect(appointmentTypesEventsCreate).toHaveBeenCalledTimes(1)
    );
    const body = vi.mocked(appointmentTypesEventsCreate).mock.calls[0][0].body;
