/**
 * AppointmentTypeFormDialog tests — edit mode and the roster-resolution helpers.
 *
 * Create mode is covered by create-appointment-type-dialog.test.tsx, which drives the same
 ...
 *   because an appointment type PATCH is only partial for `duration` and
 *   `accepts_public_scheduling`, and those two are left to
 *   PublicSchedulingSettings rather than sent from here.
 *
 * The roster helpers this form uses live in `@/lib/appointment-types/appointment-type-payload`
...
vi.mock('@/client/sdk.gen', async (importOriginal) => {
  ...
  return {
    ...
    appointmentTypesPartialUpdate: vi.fn(),
  };
});
...
/**
 * A saved appointment type whose only slot has the Nurses pool attached plus Carol picked
 * individually. Its `calendars` is the union the API reports, with no marker
 * saying which entry came from where.
 */
const SAVED_APPOINTMENT_TYPE: AppointmentType = {
  id: 42,
  name: 'Clinic',
...
function renderEditDialog(
  appointmentType: AppointmentType = SAVED_APPOINTMENT_TYPE
) {
...
function lastUpdateBody() {
  return vi.mocked(appointmentTypesPartialUpdate).mock.calls[0]?.[0]?.body;
}
...
describe('AppointmentTypeFormDialog (edit mode)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    vi.mocked(appointmentTypesPartialUpdate).mockResolvedValue({
      data: SAVED_APPOINTMENT_TYPE,
      response: new Response('{}', { status: 200 }),
    } as unknown as Awaited<ReturnType<typeof appointmentTypesPartialUpdate>>);
  });

  it('prefills the appointment type and splits the saved roster into pools and individual picks', async () => {
    renderEditDialog();

    expect(screen.getByText('Edit appointment type')).toBeInTheDocument();
  ...
  it('does not offer the public-scheduling fields in edit mode', () => {
    ...
    // appointment type exists — two editors for one setting would be one too many.
    renderEditDialog();
  ...
  it('leaves duration and accepts_public_scheduling out of the PATCH', async () => {
    ...
    // edit from also rewriting the appointment type's public-scheduling state.
    const user = userEvent.setup();
    renderEditDialog({
      ...SAVED_APPOINTMENT_TYPE,
      accepts_public_scheduling: true,
      duration: '00:45:00',
    });

    await screen.findByTestId('slot-editor-0');
    await user.click(screen.getByTestId('edit-appointment-type-submit'));

    await waitFor(() => {
      expect(appointmentTypesPartialUpdate).toHaveBeenCalledOnce();
    });
  ...
  it('PATCHes by id and sends pool_ids for every slot, even one left untouched', async () => {
    ...
    const call = vi.mocked(appointmentTypesPartialUpdate).mock.calls[0]?.[0];
  ...
  it('warns when a saved slot is deleted outright', async () => {
    ...
    renderEditDialog({
      ...SAVED_APPOINTMENT_TYPE,
      slots: [
        SAVED_APPOINTMENT_TYPE.slots[0],
        {
          ...SAVED_APPOINTMENT_TYPE.slots[0],
          id: 101,
          name: 'Room',
  ...
  it('surfaces a slot-removal rejection on the form', async () => {
    const user = userEvent.setup();
    vi.mocked(appointmentTypesPartialUpdate).mockRejectedValueOnce({
      non_field_errors: [
