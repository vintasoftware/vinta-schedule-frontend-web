...
/**
 * AppointmentTypeQuotaRules — the appointment-type-scoped quota rules for one calendar in one
 * slot (Phase 5). A rule caps how many live bookings the calendar may take
 * through this appointment type slot per day, week, or month; the model enforces one
 * rule per `(calendar, slot, period)`, so a calendar may hold a daily AND a
 * weekly rule at once (see the handoff doc, section 3).
 *
 * Structurally the plainest of the three appointment-type-scoped concepts: no
 ...
 * use-appointment-type-scoped-quota.ts's doc comment), so this file owns both the row
 * list and the add/edit form itself rather than splitting them the way
 * AppointmentTypeBlockList/AppointmentTypeBlockForm do.
 *
 ...
 *    appointment-type-scoped write uses. `readOverLimitError`/`OverLimitAlert` are
 *    reused unchanged, same as appointment-type-block-form.tsx's over-limit handling.
 *
...
import type { AppointmentTypeScopedQuotaRule, PeriodEnum } from '@/client';
...
function ruleToFormValues(
  rule: AppointmentTypeScopedQuotaRule
): QuotaRuleFormValues {
...
interface AppointmentTypeQuotaRuleFormProps {
...
function AppointmentTypeQuotaRuleForm({
  appointmentTypeId,
  slotId,
  calendarId,
  rule,
  onSaved,
  onCancel,
}: AppointmentTypeQuotaRuleFormProps) {
  const isEdit = rule !== undefined;

  const { createQuotaRule, updateQuotaRule } = useAppointmentTypeScopedQuota({
    appointmentTypeId,
    slotId,
    calendarId,
    // The list this hook would otherwise fetch is already loaded by the
    // caller (AppointmentTypeQuotaRules uses the same hook, same query key, so
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Maximum live bookings this calendar may take through this
                appointment type slot within one period.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
...
interface QuotaRuleRowProps {
  ...
  readOnly: boolean;
  isDeleting: boolean;
  onEdit: (rule: AppointmentTypeScopedQuotaRule) => void;
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {/* Confirm on every delete: removing a quota rule uncaps the
                    ...
                    all future bookings (same reasoning as appointment-type-block-list's
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                // can only be set through className (see appointment-type-block-list.tsx).
...
export interface AppointmentTypeQuotaRulesProps {
  appointmentTypeId: number;
  slotId: number;
  calendarId: number;
}

export function AppointmentTypeQuotaRules({
  appointmentTypeId,
  slotId,
  calendarId,
}: AppointmentTypeQuotaRulesProps) {
  ...
  const { rules, isLoading, isTruncated, deleteQuotaRule } =
    useAppointmentTypeScopedQuota({ appointmentTypeId, slotId, calendarId });

  const [dialogOpen, setDialogOpen] = React.useState(false);
  const [editingRule, setEditingRule] =
    React.useState<AppointmentTypeScopedQuotaRule | null>(null);
  ...
  const openEdit = (rule: AppointmentTypeScopedQuotaRule) => {
    setEditingRule(rule);
    setDialogOpen(true);
  };
  ...
  const handleDelete = React.useCallback(
    async (id: number) => {
      ...
      try {
        const result = await deleteQuotaRule({
          appointmentTypeId,
          slotId,
          ruleId: id,
        });
    ...
    [deleteQuotaRule, appointmentTypeId, slotId]
  );
  ...
  return (
    <VStack gap={3} data-testid='appointment-type-quota-rules'>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Caps bookings made through this appointment type slot only --
                base availability and every other appointment type are
                unaffected.
              </DialogDescription>
            </DialogHeader>
            {/* Conditionally mounted (not just visually hidden) so switching
                between "create" and editing rule A vs rule B always gives
                AppointmentTypeQuotaRuleForm a fresh mount -- same convention as
                appointment-type-block-list.tsx's AppointmentTypeBlockForm mount. */}
            {dialogOpen && (
              <AppointmentTypeQuotaRuleForm
