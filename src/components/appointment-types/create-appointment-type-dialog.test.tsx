/**
 * CreateAppointmentTypeDialog tests.
 *
 * Covers:
 * - Building an appointment type with 2 slots (names, required counts, individually picked
 *   calendars) → submit calls appointmentTypesCreate with the correct nested body.
...
function mockAppointmentTypeCreate(result?: Partial<AppointmentType>) {
  const appointmentType: AppointmentType = {
    id: 42,
    name: 'Test Appointment Type',
    description: '',
    slots: [],
    public_booking_slug: 'test-appointment-type',
    created: '2024-01-01T00:00:00Z',
    modified: '2024-01-01T00:00:00Z',
    ...result,
  };
  vi.mocked(appointmentTypesCreate).mockResolvedValue({
    data: appointmentType,
    response: new Response('{}', { status: 201 }),
  } as unknown as Awaited<ReturnType<typeof appointmentTypesCreate>>);
}
...
function renderDialog(open = true) {
  ...
  const result = render(
    <CreateAppointmentTypeDialog open={open} onOpenChange={onOpenChange} />,
    { wrapper }
  );
...
describe('CreateAppointmentTypeDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockCalendarList();
    mockPoolList();
  ...
  it('renders the dialog with form fields', async () => {
    ...
    expect(screen.getByText('New appointment type')).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText('e.g. Frontend Team')
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText('What is this appointment type used for?')
    ).toBeInTheDocument();
  ...
  it('builds an appointment type with 2 slots and correct rosters → calls appointmentTypesCreate with the right body', async () => {
    const user = userEvent.setup();
    mockAppointmentTypeCreate();
    ...
    // Submit
    const submitBtn = screen.getByTestId('create-appointment-type-submit');
    await user.click(submitBtn);

    await waitFor(() => {
      expect(appointmentTypesCreate).toHaveBeenCalledOnce();
    });

    const callBody = vi.mocked(appointmentTypesCreate).mock.calls[0]?.[0]?.body;
    expect(callBody).toMatchObject({
      name: 'My New Appointment Type',
      slots: [
        {
          name: 'Interviewers',
          required_count: 2,
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          description: expect.stringContaining('My New Appointment Type'),
        })
      );
    });
  ...
  it('blocks submit when a slot has an empty roster', async () => {
    ...
    // Fill only appointment type name + slot name, but leave the roster empty
    await user.type(
      screen.getByPlaceholderText('e.g. Frontend Team'),
      'Empty Pool Appointment Type'
    );
    const slot0 = screen.getByTestId('slot-editor-0');
    await user.type(
      within(slot0).getByPlaceholderText('e.g. Interviewer'),
    ...
    // appointmentTypesCreate should NOT have been called
    expect(appointmentTypesCreate).not.toHaveBeenCalled();
  });
  ...
  it('blocks submit when required_count exceeds the roster size', async () => {
    ...
    // Submit
    await user.click(screen.getByTestId('create-appointment-type-submit'));
  ...
  it('sets accepts_public_scheduling and duration at creation time', async () => {
    // Regression: these two were unsettable at creation, so a new appointment type could
    ...
    expect(
      vi.mocked(appointmentTypesCreate).mock.calls[0]?.[0]?.body
    ).toMatchObject({
      name: 'Clinic',
  ...
  it('blocks accepting public bookings with no appointment length', async () => {
    // The server rejects a public appointment type with no duration; catch it here so the
  ...
  it('omits duration entirely when no length was typed', async () => {
    ...
    // so a private appointment type with no length sends no key at all.
    const user = userEvent.setup();
    mockAppointmentTypeCreate();
    renderDialog();
    ...
    const body = vi.mocked(appointmentTypesCreate).mock.calls[0]?.[0]?.body as
  ...
  it('shows a toast error if appointmentTypesCreate throws', async () => {
    const user = userEvent.setup();
    vi.mocked(appointmentTypesCreate).mockRejectedValueOnce(
