...
/**
 * AppointmentTypeFormDialog — build or edit an Appointment Type, shared by the create and
 * edit entry points.
 *
 ...
 * `PublicSchedulingSettings` on the appointment type detail page, which owns the same two
 * fields plus the appointment type's public link — two editors for one setting would be
 * one too many. Both honour the server invariant that an appointment type accepting public
 ...
 *     carries is removed, which drops that slot's appointment-type-scoped availability
...
import {
  ...
  type PoolRosters,
} from '@/lib/appointment-types/appointment-type-payload';
import {
...
export function createAppointmentTypeFormSchema(rosters: PoolRosters) {
  const slotSchema = z
    .object({
  ...
  return z
    .object({
      name: z
        .string()
        .trim()
        .min(1, { message: 'Appointment Type name is required' }),
    ...
    .refine(
      ...
      {
        message:
          'Set an appointment length before accepting public bookings — the server rejects a public appointment type with no duration.',
...
export type AppointmentTypeFormValues = z.infer<
  ReturnType<typeof createAppointmentTypeFormSchema>
>;
...
function getDefaultValues(
  ...
  return {
    ...
    // Seeded so the schema's cross-field rule reads the appointment type's real state,
    // but never submitted from edit mode — see the header.
    acceptsPublicScheduling: Boolean(appointmentType.accepts_public_scheduling),
    durationMinutes: djangoDurationToMinutes(appointmentType.duration),
    slots: appointmentType.slots.map((slot) => ({
      name: slot.name,
...
export interface AppointmentTypeFormDialogProps {
  ...
  /** The appointment type being edited; omit or pass null to create a new one. */
  appointmentType?: AppointmentType | null;
}
...
export function AppointmentTypeFormDialog({
  ...
  const resolver = React.useMemo(
    () => zodResolver(createAppointmentTypeFormSchema(rosters)),
    [rosters]
  );

  const { createAppointmentType, createAppointmentTypeMutation } =
    useCreateAppointmentType();
  const { updateAppointmentType, updateAppointmentTypeMutation } =
    useUpdateAppointmentType();

  const form = useForm<AppointmentTypeFormValues>({
    resolver,
    defaultValues: getDefaultValues(appointmentType),
  });
  ...
  // Re-seed on open, and whenever a different appointment type is handed in — the appointment types
  // table reuses one mounted dialog across every row's Edit action.
  React.useEffect(() => {
    form.reset(getDefaultValues(appointmentType));
  ...
  const droppedSlots = isEdit
    ? appointmentType.slots
        .map((s) => s.name)
        .filter((n) => !currentSlotNames.has(n))
    : [];
  ...
  const onSubmit = async (values: AppointmentTypeFormValues) => {
    ...
    try {
      if (isEdit) {
        // Through buildAppointmentTypeUpdateBody so name/description/slots are always
        // carried — an appointment type PATCH is only partial for duration and
        ...
        await updateAppointmentType(
          appointmentType.id,
          buildAppointmentTypeUpdateBody(appointmentType, {
            name: values.name,
        ...
        await createAppointmentType({
          ...
          // Sent whenever a length was typed, not only when the appointment type is
          // public: a private appointment type can carry one now and be flipped public
        ...
        <Form {...form}>
          ...
          <FormLayout onSubmit={form.handleSubmit(onSubmit)} gap={4} noValidate>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Appointment Type name</FormLabel>
                  <FormControl>
                    <Input
                      type='text'
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type='text'
                      placeholder='What is this appointment type used for?'
                      autoComplete='off'
                      {...field}
                    />
                  </FormControl>
                      ...
                      <HStack gap={4} align='center' justify='between'>
                        <Box grow>
                          ...
                          <FormDescription>
                            ...
                            link can book without a code. You can change this
                            later.
                          </FormDescription>
                        </Box>
                        <FormControl>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        ...
                        booking made through this appointment type&apos;s links.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            {droppedSlots.length > 0 ? (
              ...
              >
                ...
                <Text size='xs' color='muted-foreground'>
                  ...
                  new one. Any appointment-type-scoped availability windows,
                  blocked time, and quota rules on{' '}
                  {droppedSlots.length === 1 ? 'it' : 'them'} are lost, and the
                  save is refused while{' '}
            ...
            <DialogFooter>
              ...
              <Button
                ...
                data-testid={
                  isEdit
                    ? 'edit-appointment-type-submit'
                    : 'create-appointment-type-submit'
                }
              >
                {isPending
                  ? isEdit
...
interface SlotEditorProps {
  index: number;
  form: UseFormReturn<AppointmentTypeFormValues>;
  calendars: Calendar[];
