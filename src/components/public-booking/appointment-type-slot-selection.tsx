...
/**
 * AppointmentTypeSlotSelection — per-slot calendar selection for a public appointment type
 * booking link, once the attendee has picked a whole-appointment-type time proposal.
 *
 * Reuses the exact `SlotViewModel` shape and the pure `isSelectionComplete`
 * / `hasUnsatisfiableSlot` helpers from `@/lib/booking-links/appointment-type-selection`
 * (moved there in this phase specifically so this public surface could use
 * them without dragging the authenticated appointment-type-booking hook's imports into
 ...
 * "busy", mirroring the authenticated appointment-type-booking dialog's per-slot picker
 * (`@/components/appointment-types/appointment-type-booking-flow.tsx`).
 *
 * PRIVACY: this surface is unauthenticated. The public
 * `appointment-type-availability` read returns ONLY numeric calendar ids per
...
import type { AppointmentTypeSlotSelectionInput } from '@/client';
import {
  isSelectionComplete,
...
export interface AppointmentTypeSlotSelectionProps {
  slots: SlotViewModel[];
  ...
  onSubmit: (slotSelections: AppointmentTypeSlotSelectionInput[]) => void;
  isSubmitting?: boolean;
  /** Return to time-proposal selection — omitted hides the button. */
  onBack?: () => void;
}

export function AppointmentTypeSlotSelection({
  slots,
  selection,
  onToggle,
  onSubmit,
  isSubmitting = false,
  onBack,
}: AppointmentTypeSlotSelectionProps) {
  ...
  const handleSubmit = () => {
    if (!canSubmit) return;
    const slotSelections: AppointmentTypeSlotSelectionInput[] = slots.map(
      (slot) => ({
        slot_id: slot.slotId,
  ...
  return (
    <VStack gap={4} data-testid='appointment-type-slot-selection'>
      {unsatisfiable ? (
        <Alert
          variant='warning'
          data-testid='appointment-type-slot-unsatisfiable-alert'
        >
      ...
      {slots.map((slot) => {
        ...
        return (
          <VStack
            ...
            border
            radius='md'
            data-testid={`appointment-type-slot-${slot.slotId}`}
          >
            <HStack gap={2} align='center' justify='between'>
              ...
              <Badge
                ...
                data-testid={`appointment-type-slot-${slot.slotId}-progress`}
              >
            ...
            {!slotSatisfiable ? (
              <Text
                size='xs'
                color='destructive'
                data-testid={`appointment-type-slot-${slot.slotId}-unsatisfiable`}
              >
            ...
            <VStack gap={2}>
              {slot.pool.map((candidate, index) => {
                ...
                const inputId = `appointment-type-slot-${slot.slotId}-option-${candidate.id}`;
                return (
      ...
      <HStack gap={2} justify='end'>
        {onBack ? (
          <Button
            ...
            data-testid='appointment-type-slot-selection-back'
          >
            Back
          </Button>
        ) : null}
        <Button
          ...
          disabled={!canSubmit}
          data-testid='appointment-type-slot-selection-continue'
        >
