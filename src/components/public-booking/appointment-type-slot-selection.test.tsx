/**
 * AppointmentTypeSlotSelection tests.
 *
 * Covers (per the phase spec):
 * - A satisfiable appointment type completes and calls `onSubmit` with the right
...
/**
 * `AppointmentTypeSlotSelection` is a controlled component (selection state lives in
 ...
 * real flow parent (`public-appointment-type-booking-flow.tsx`) does.
 */
...
describe('AppointmentTypeSlotSelection', () => {
  it('a satisfiable appointment type completes and submits the right slot_selections', async () => {
    ...
    await user.click(screen.getByTestId('appointment-type-slot-1-option-10'));
  ...
  it('an unsatisfiable slot hard-blocks submit even with a complete-looking selection elsewhere', () => {
    ...
    expect(
      screen.getByTestId('appointment-type-slot-3-unsatisfiable')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('appointment-type-slot-selection-continue')
    ).toBeDisabled();
  ...
  it('a busy candidate is not selectable', async () => {
    ...
    const busyCheckbox = screen.getByTestId(
      'appointment-type-slot-2-option-21'
    );
    ...
    const freeCheckbox = screen.getByTestId(
      'appointment-type-slot-2-option-20'
    );
