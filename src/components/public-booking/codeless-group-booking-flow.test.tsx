/**
 * CodelessGroupBookingFlow integration tests.
 *
 * Covers (per Phase 7 spec):
 * - A public group's page shows slots and books with NO code, and no
 *   request carries `X-Booking-Code`.
 * - An unknown slug (404) renders `<CodelessGroupNotFound />`; a real but
 *   non-public group (403) renders `<CodelessGroupUnavailable />` — the two
 *   are distinct.
 ...
 * - `409 SLOT_UNAVAILABLE` on submit returns to whole-group time selection
...
): Awaited<ReturnType<typeof publicBookingCalendarGroupsBookableSlotsList>> {
  return {
    data: proposals,
...
): Awaited<ReturnType<typeof publicBookingCalendarGroupsAvailabilityCreate>> {
...
describe('CodelessGroupBookingFlow', () => {
  ...
  it('renders CodelessGroupNotFound (not CodelessGroupUnavailable) for an unknown slug (404)', async () => {
    vi.mocked(
      publicBookingCalendarGroupsBookableSlotsList
    ).mockResolvedValueOnce(proposalsFailed(404, { detail: 'Not found.' }));

    renderFlow('no-such-group');

    await waitFor(() =>
      expect(screen.getByTestId('codeless-group-not-found')).toBeInTheDocument()
    );
    expect(
      screen.queryByTestId('codeless-group-unavailable')
    ).not.toBeInTheDocument();
  });

  it('renders CodelessGroupUnavailable (not CodelessGroupNotFound) for a real, non-public group (403) — distinct from the 404 state', async () => {
    vi.mocked(
      publicBookingCalendarGroupsBookableSlotsList
    ...
    renderFlow('private-group');

    await waitFor(() =>
      expect(
        screen.getByTestId('codeless-group-unavailable')
      ).toBeInTheDocument()
    );
    expect(
      screen.queryByTestId('codeless-group-not-found')
    ).not.toBeInTheDocument();
  });
  ...
  it('SLOT_UNAVAILABLE on submit returns to whole-group time selection with the proposal list refetched', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('codeless-group-slot-unavailable-notice')
      ).toBeInTheDocument()
    );
  ...
  it.each([
    ...
    async (errorCode, status, expectedPhrase) => {
      ...
      await waitFor(() =>
        expect(
          screen.getByTestId('codeless-group-booking-terminal-error')
        ).toBeInTheDocument()
      );
      expect(
