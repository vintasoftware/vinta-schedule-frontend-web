/**
 * CodelessGroupBookingPage is an async Server Component — render the
...
import CodelessGroupBookingPage, { metadata } from './page';

describe('CodelessGroupBookingPage (/g/[public_slug])', () => {
  ...
  it('renders default (vinta) branding — a bare slug has no org to look up', async () => {
    render(
      await CodelessGroupBookingPage({
  ...
  it('passes public_slug through to CodelessGroupBookingFlow, with no org slug', async () => {
    render(
      await CodelessGroupBookingPage({
        params: Promise.resolve({ public_slug: 'surgery-team' }),
      })
