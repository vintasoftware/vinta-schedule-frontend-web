/**
 * BookReschedulePage is an async Server Component — render the awaited
 * element directly, same pattern as `/book/[code]/page.test.tsx`.
 */
...
import BookReschedulePage, { metadata } from './page';

describe('BookReschedulePage (/book/[code]/reschedule)', () => {
  ...
  it('renders default (vinta) branding — a bare code has no org to look up', async () => {
    render(
      await BookReschedulePage({ params: Promise.resolve({ code: 'abc123' }) })
    );
  ...
  it('passes the code through to RescheduleFlow', async () => {
    render(
      await BookReschedulePage({ params: Promise.resolve({ code: 'abc123' }) })
    );
