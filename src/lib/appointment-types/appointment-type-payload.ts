/**
 * appointment-type-payload.ts — the one place a `AppointmentType` write body is built.
 *
 * **`PATCH /appointment-types/{id}/` is only partial for two of its fields.**
 * `AppointmentTypeSerializer` reads the rest of the body as a whole-object
 ...
 *
 * `buildAppointmentTypeUpdateBody` takes the appointment type as last read plus the fields being
 * changed, and returns a body that satisfies all of that. Every caller that
 * PATCHes an appointment type should go through it rather than hand-assembling a body.
 */
...
import type {
  ...
  AppointmentType,
  AppointmentTypeSlot,
  AppointmentTypeSlotWritable,
  CalendarPool,
  PatchedAppointmentTypeWritable,
} from '@/client';
 ...
 * loading, and the appointment type form blocks submit until it has resolved.
 */
...
/** Every slot of a saved appointment type, as the writable list that reproduces it. */
export function savedSlotsToWritable(
  appointmentType: AppointmentType
): AppointmentTypeSlotWritable[] {
  return appointmentType.slots.map(savedSlotToWritable);
}
...
/**
 * The fields a caller may change. Anything omitted here keeps the appointment type's
 ...
 */
export interface AppointmentTypeUpdateChanges {
  name?: string;
  description?: string;
  slots?: AppointmentTypeSlotWritable[];
  duration?: string;
  accepts_public_scheduling?: boolean;
}

/**
 * Builds a complete `PATCH /appointment-types/{id}/` body from the appointment type as last
 * read plus the fields being changed.
 *
 * `name`, `description` and `slots` are always present — carried over from
 * `appointment type` when the caller isn't changing them. `duration` and
 ...
 */
export function buildAppointmentTypeUpdateBody(
  appointmentType: AppointmentType,
  changes: AppointmentTypeUpdateChanges = {}
): PatchedAppointmentTypeWritable {
  const body: PatchedAppointmentTypeWritable = {
    name: changes.name ?? appointmentType.name,
    description: changes.description ?? appointmentType.description ?? '',
    slots: changes.slots ?? savedSlotsToWritable(appointmentType),
  };
