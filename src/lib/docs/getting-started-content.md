...
4. Note the token value — it is shown only once. Store it securely (e.g., in an environment variable)

Your token takes the form: `<system_user_id>:<secret_token>`

For example: `user-123:sk_live_abc123xyz...`

## Step 2: Send the Authorization Header

Every API request must include an `Authorization` header with your token in this exact format:
...
```
Authorization: Bearer user-123:sk_live_abc123xyz...
```
...
curl -X POST \
  ...
  -d '{
    "query": "query { calendarGroupBookableSlots(groupId: 42, searchWindowStart: \"2026-06-17T09:00:00-07:00\", durationSeconds: 1800) { startTime endTime } }"
  }'
```

Replace `<system_user_id>:<token>` with your actual token and `api.example.com` with your API base URL.
...
This query searches a calendar group (groupId: 42) for slots of 30 minutes (1800 seconds) that start on or after the specified timestamp.
...
This mutation books the selected slot across every calendar in the group, writes it to all connected providers (Google Calendar, Exchange, etc.), and returns the created event details.
