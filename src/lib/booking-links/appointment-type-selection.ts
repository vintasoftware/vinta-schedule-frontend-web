/**
 * Pure, framework-free appointment-type slot-selection helpers.
 *
 * Originally lived inline in `@/hooks/appointment-types/use-appointment-type-booking.ts`
 * (the authenticated appointment-type-booking hook). Extracted here in Phase 3 of the
 ...
 * `@/client/sdk.gen`'s `appointmentTypesBookableSlotsList` /
 * `appointmentTypesAvailabilityCreate` / `appointmentTypesEventsCreate`, and
 ...
 * `use-appointment-type-booking.ts` and mounted under `/book/[code]` produced a
 ...
 * eliminate `use-appointment-type-booking.ts`'s unused exports/imports at this
 ...
 * compile time).
 *
 * `use-appointment-type-booking.ts` re-exports everything here unchanged, so the
 * authenticated appointment-type-booking flow and its existing tests
 * (`@/components/appointment-types/`, `use-appointment-type-booking.test.ts`) see no
...
export function slotRequiredCount(slot: AppointmentTypeSlot): number {
...
/**
 * Map an AppointmentTypeRangeAvailability (per-range, per-slot) onto the appointment type's
...
export function buildSlotAvailability(
  slots: AppointmentTypeSlot[],
  rangeAvailability: AppointmentTypeRangeAvailability | undefined
): SlotAvailability[] {
