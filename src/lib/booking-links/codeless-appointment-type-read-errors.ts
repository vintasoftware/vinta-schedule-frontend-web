/**
 * Error taxonomy for the CODELESS, slug-addressed appointment-type reads
 * (`GET /public/booking/appointment-types/{public_slug}/bookable-slots/` and
 * `POST .../availability/`).
 *
 ...
 *      - An unknown slug is a real `404`. Confirming "no such appointment type" discloses
 *        nothing the caller didn't already risk by guessing/mistyping a slug.
 *      - A slug resolving to a real but non-public appointment type (or a public appointment type
 *        whose effective duration is unset — `_resolve_public_group_duration`
 *        fails closed the same way) is a real `403`: the appointment type exists, but
...
/**
 * `'not-found'` = unknown `public_slug` (404); `'unavailable'` = a real appointment type
 ...
 */
export type CodelessAppointmentTypeReadState =
  | 'ok'
  | 'not-found'
  | 'unavailable'
  | 'range-invalid'
...
/**
 * Map a failed codeless appointment type read response's status to a
 * `CodelessAppointmentTypeReadState`. Takes only the `Response` — there is no body to
 ...
 */
export function parseCodelessAppointmentTypeReadFailure(
  response: Response
): CodelessAppointmentTypeReadState {
...
/**
 * Thrown by the codeless appointment type read hooks (`use-codeless-appointment-type-booking.ts`)
 * so a failed `useQuery` (or the imperative availability fetch) carries the
 * already-mapped `CodelessAppointmentTypeReadState` in `.state`. Components branch on
 * `.state` to choose between `<CodelessAppointmentTypeNotFound />` and
 * `<CodelessAppointmentTypeUnavailable />` — never on the raw response body.
 */
export class CodelessAppointmentTypeReadFailureError extends Error {
  readonly state: Exclude<CodelessAppointmentTypeReadState, 'ok'>;

  constructor(state: Exclude<CodelessAppointmentTypeReadState, 'ok'>) {
    super(`codeless appointment type read failed: ${state}`);
    this.name = 'CodelessAppointmentTypeReadFailureError';
    this.state = state;
  }
}
