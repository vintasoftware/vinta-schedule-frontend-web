/**
 * appointment-type-selection.ts tests.
 *
 * This module's pure logic is exercised in much greater depth (fixtures,
 * edge cases) by `@/hooks/appointment-types/use-appointment-type-booking.test.ts`, which
...
const SLOT_A: AppointmentTypeSlot = {
  id: 1,
  name: 'Slot A',
  required_count: 2,
  calendars: [
    { id: 10, name: 'Cal 10' } as AppointmentTypeSlot['calendars'][number],
    { id: 11, name: 'Cal 11' } as AppointmentTypeSlot['calendars'][number],
    { id: 12, name: 'Cal 12' } as AppointmentTypeSlot['calendars'][number],
  ],
  pools: [],
};
...
describe('buildSlotAvailability', () => {
  it('intersects reported free ids with the slot pool and computes satisfiability', () => {
    const rangeAvailability: AppointmentTypeRangeAvailability = {
