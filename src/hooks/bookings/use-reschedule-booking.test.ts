/**
 * useRescheduleBooking tests.
 *
 * Covers:
 *  - Non-recurring event → calendarEventsPartialUpdate with new start_time/end_time/timezone.
 *  - Recurring event, scope='all' → calendarEventsPartialUpdate (same as non-recurring).
 *  - Recurring event, scope='this' → calendarEventsCreateExceptionCreate with modified times
 *    and the correct exception_date.
 *  - Recurring event, scope='following' → calendarEventsBulkModifyCreate with
 *    modification_start_date and time offsets.
 *  - invalidateCalendarEvents is called after every successful reschedule.
...
// New start/end for the reschedule (moved 1 hour later, same day).
...
describe('useRescheduleBooking', () => {
  ...
  describe('non-recurring event', () => {
    it('calls calendarEventsPartialUpdate with new start_time, end_time, and timezone', async () => {
  ...
  describe('recurring event, scope = "all"', () => {
    it('calls calendarEventsPartialUpdate with new times (same as non-recurring)', async () => {
      vi.mocked(calendarEventsPartialUpdate).mockResolvedValue(
        makeOkResponse()
      );
  ...
  describe('recurring event, scope = "this"', () => {
    it('calls calendarEventsCreateExceptionCreate with modified_start_time and modified_end_time', async () => {
    ...
    it('does NOT call partialUpdate or bulkModify', async () => {
      vi.mocked(calendarEventsCreateExceptionCreate).mockResolvedValue(
        makeOkResponse()
      );
  ...
  describe('recurring event, scope = "following"', () => {
    it('calls calendarEventsBulkModifyCreate with modification_start_date and time offsets', async () => {
      ...
      // Time offsets in HH:MM:SS format.
    ...
    it('does NOT call partialUpdate or createException', async () => {
      vi.mocked(calendarEventsBulkModifyCreate).mockResolvedValue(
        makeOkResponse()
      );
  ...
  describe('error propagation', () => {
    it('propagates backend errors to the caller', async () => {
      ...
      await expect(
        result.current.rescheduleBooking(
          makeEventVM(),
          NEW_START,
          NEW_END,
          TIMEZONE
