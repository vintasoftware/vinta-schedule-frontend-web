/**
 * useCodelessGroupBookableSlots — dedicated public-client regression test.
 *
 * Same rationale as `use-public-group-booking.public-client.test.ts`: the
 ...
 * group link), points `publicBookingClient` at a mock `fetch`, and asserts
 ...
 * server-side (see `use-codeless-group-booking.ts`'s doc comment). A
...
import { useCodelessGroupBookableSlots } from './use-codeless-group-booking';
...
describe('useCodelessGroupBookableSlots (real publicBookingClient)', () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it('sends no X-Booking-Code, Authorization, or X-Organization-Id, even with a token and active org in localStorage', async () => {
    // Simulate a logged-in visitor browsing a public group's reusable link.
    ...
    expect(outgoingRequest.url).toContain(
      '/public/booking/calendar-groups/surgery-team/bookable-slots/'
    );
