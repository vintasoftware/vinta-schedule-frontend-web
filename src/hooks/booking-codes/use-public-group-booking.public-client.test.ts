/**
 * usePublicGroupBookableSlots — dedicated public-client regression test.
 *
 * `use-public-group-booking.test.ts` mocks `@/client/sdk.gen` at the module
...
import { usePublicGroupBookableSlots } from './use-public-group-booking';
...
describe('usePublicGroupBookableSlots (real publicBookingClient)', () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it('sends X-Booking-Code through the real public client, with no Authorization or X-Organization-Id even with a token and active org in localStorage', async () => {
    // Simulate a logged-in visitor browsing a public group link.
