 ...
 *     closes.
 *   - `gcTime: 0` keeps `code` out of the *mutation* cache too: with the
 *     app's default 5-minute mutation `gcTime`
 *     (@/components/query-client-provider.tsx), `Mutation.state.data.code`
 *     would otherwise stay readable via `queryClient.getMutationCache()` for
 *     up to five minutes after the dialog closes, even though the local
 *     component state was cleared — `MutationObserver.reset()` only calls
 *     `removeObserver()` -> `scheduleGc()`, which defers to `gcTime`, it does
 *     not remove the mutation synchronously. With `gcTime: 0`,
 *     `optionalRemove()` fires as soon as the last observer detaches, and
 *     `addObserver` clears any pending timer, so an in-flight request or an
 *     open dialog is unaffected.
 *   - Never logged. Never persisted to `localStorage` / `sessionStorage`.
 *   - `retry: 0`: a retried mint after the server already committed a create
 *     would mint a *second* live code that the UI never reveals — and since
 *     the API exposes no list/retrieve for booking codes, that orphaned code
 *     could never be revoked through this app. Fail once and let the member
