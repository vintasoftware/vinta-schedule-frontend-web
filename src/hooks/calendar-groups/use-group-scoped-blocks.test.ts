...
import {
  useGroupScopedBlocks,
  GROUP_SCOPED_BLOCKS_PAGE_SIZE,
} from './use-group-scoped-blocks';
...
describe('useGroupScopedBlocks', () => {
  ...
  describe('list', () => {
    it('requests the list by group_id/slot_id path params', async () => {
      ...
      const { result } = renderHook(
        () => useGroupScopedBlocks({ groupId: 1, slotId: 10 }),
        { wrapper: Wrapper }
      );
    ...
    it('filters to one calendar client-side when calendarId is set', async () => {
      ...
      const { Wrapper } = makeQueryWrapper();
      const { result } = renderHook(
        () => useGroupScopedBlocks({ groupId: 1, slotId: 10, calendarId: 42 }),
        { wrapper: Wrapper }
      );
  ...
  describe('createBlock', () => {
    it('round-trips reason through the create body and the unwrapped result', async () => {
      ...
      expect(
        vi.mocked(calendarGroupsSlotsBlockedTimesCreate)
      ).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('invalidates the slot list query on success', async () => {
      ...
      await waitFor(() => expect(result.current.isLoading).toBe(false));
      const callsBeforeMutation = vi.mocked(calendarGroupsSlotsBlockedTimesList)
        .mock.calls.length;
      ...
      await waitFor(() => {
        expect(
          vi.mocked(calendarGroupsSlotsBlockedTimesList).mock.calls.length
  ...
  describe('updateBlock', () => {
    ...
    it('round-trips an updated reason through the unwrapped result', async () => {
      ...
      expect(
        vi.mocked(calendarGroupsSlotsBlockedTimesPartialUpdate)
      ).toHaveBeenCalledWith(
        expect.objectContaining({
          body: { reason: 'Rescheduled' },
        })
  ...
  describe('deleteBlock', () => {
    ...
    it('rejects on a transport failure (500) rather than returning row_gone', async () => {
      ...
      await expect(
        result.current.deleteBlock({ groupId: 1, slotId: 10, blockId: 501 })
      ).rejects.toThrow();
    });
  });
});
