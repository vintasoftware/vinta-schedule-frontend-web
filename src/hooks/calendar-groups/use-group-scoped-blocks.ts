/**
 * useGroupScopedBlocks — list, create, update, and delete group-scoped
 * blocked times for one slot (optionally scoped to one calendar).
 *
 * REST surface: `.../calendar-groups/{group_id}/slots/{slot_id}/blocked-times/`
 * (see the handoff doc, `ai-plans/2026-08-05-CALENDAR_GROUP_SCOPED_AVAILABILITY_HANDOFF.md`).
 * Mirrors use-group-scoped-windows.ts exactly, with `reason` added to the
 ...
 * or a narrowing update) — see the handoff doc's "4.2 Group-scoped blocked
 ...
 * back, same as the windows hook does. The caller (group-block-form.tsx)
...
import { calendarGroupsSlotsBlockedTimesDestroy } from '@/client';
import type {
...
// The `_id` the generated factory tags every calendarGroupsSlotsBlockedTimesList*
...
export const GROUP_SCOPED_BLOCKS_LIST_OPERATION_ID =
  'calendarGroupsSlotsBlockedTimesList';

// Matches use-group-scoped-config-summary.ts's SUMMARY_PAGE_SIZE and
// use-group-scoped-windows.ts's GROUP_SCOPED_WINDOWS_PAGE_SIZE (same query
...
export const GROUP_SCOPED_BLOCKS_PAGE_SIZE = 200;

export interface GroupScopedBlockWriteResultUnwrapped {
  block: GroupScopedBlockedTime;
  orphanedBookings: GroupScopedBlockOrphanedBooking[];
}
...
export interface UseGroupScopedBlocksOptions {
  groupId: number;
  slotId: number;
...
export function useGroupScopedBlocks({
  groupId,
  slotId,
  calendarId,
  enabled = true,
}: UseGroupScopedBlocksOptions) {
  const queryClient = useQueryClient();

  const blocksQuery = useQuery({
    ...calendarGroupsSlotsBlockedTimesListOptions({
      path: { group_id: groupId, slot_id: slotId },
      query: { limit: GROUP_SCOPED_BLOCKS_PAGE_SIZE },
    }),
    enabled,
  });
    ...
    queryClient.invalidateQueries({
      predicate: (query) =>
        ...
        (query.queryKey[0] as { _id?: string })?._id ===
          GROUP_SCOPED_BLOCKS_LIST_OPERATION_ID,
    });
  }, [queryClient]);
  ...
  ): Promise<GroupScopedBlockWriteResultUnwrapped> => {
  ...
  const deleteBlockMutation = useMutation({
    mutationFn: async (input: DeleteBlockInput): Promise<DeleteBlockResult> => {
      const { response } = await calendarGroupsSlotsBlockedTimesDestroy({
        path: {
          group_id: input.groupId,
          slot_id: input.slotId,
          id: String(input.blockId),
      ...
      if (!response) {
        throw new Error('Failed to delete group-scoped block (no response)');
      }
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to delete group-scoped block (${response.status})`
        );
      }
  ...
  return {
     ...
     */
    isTruncated: (blocksQuery.data?.count ?? 0) > GROUP_SCOPED_BLOCKS_PAGE_SIZE,
