 ...
 * group-scoped windows for that date's weekday (`useGroupScopedWindows`,
 * the same data `group-window-grid.tsx` renders as the weekday grid,
 ...
 *
 * A day whose weekday has NO representable group-scoped window for this
 ...
 * `calendar_group_service.py`'s `check_group_availability` docstring) and
 ...
 * fire until then. The group-scoped windows list this hook also reads is
 * gated by the same `enabled` flag; in practice it's a cache hit (not a new
 * network request) because `group-window-grid.tsx` already loads the same
 * groupId/slotId windows list for this page, but gating it here keeps that
...
import { useGroupScopedWindows } from './use-group-scoped-windows';
import {
  classifyWindows,
  weekdayMatrix,
  type BydayCode,
  type WeekdayWindow,
} from '@/components/calendar-groups/group-scoped-types';
...
export interface DayPlan {
  ...
  date: string;
  /**
   * One probe per representable group-scoped window whose weekday matches
   ...
   * group-scoped window for this weekday in this slot — see the module doc
...
/**
 * Reduces the group availability response onto `dayPlans`, answering
 * per-day whether `calendarId` comes back free for `slotId`.
 *
 * A day with no probes at all (no representable group-scoped window for
 ...
 * convention as use-group-booking.ts's `fetchSlotAvailability` — so a
...
export function useGroupAvailabilityPreview({
  ...
  const windowsResult = useGroupScopedWindows({
    groupId,
    slotId,
    calendarId,
    enabled,
