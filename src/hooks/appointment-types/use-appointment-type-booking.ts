/**
 * useAppointmentTypeBooking — orchestrates booking an event through an Appointment Type.
 *
 * An Appointment Type is a set of named *slots*. Each slot has:
 ...
 * Booking an appointment type event means picking, for EACH slot, exactly `required_count`
 * calendars from its pool — and only ones that are *free* for the chosen time.
 *
 * This hook wraps three appointment type operations and maps the nested API model into a
 * flat, UI-drivable shape:
 *
 *  1. `appointmentTypesBookableSlotsList` — suggests bookable time ranges for a
 *     desired duration within a search window. These are whole-appointment-type proposals
 *     (a start/end the appointment type as a whole can satisfy), used to seed the picker.
 *
 *  2. `appointmentTypesAvailabilityCreate` — given one or more [start,end] ranges,
 ...
 *
 *  3. `appointmentTypesEventsCreate` — records the booking. The body carries
 ...
 * `@/lib/booking-links/appointment-type-selection.ts` — re-exported below unchanged —
 * because the public (unauthenticated) appointment type booking flow needs
 ...
 * (`@/hooks/events/use-calendar-events`, the `@/client/sdk.gen` appointment type
...
export {
  ...
  type SlotViewModel,
} from '@/lib/booking-links/appointment-type-selection';

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

/** Params for fetching whole-appointment-type bookable time suggestions. */
...
export interface BookAppointmentTypeEventParams {
  appointmentTypeId: number;
  title: string;
  description?: string;
  startTime: string;
...
export function useAppointmentTypeBooking() {
  const queryClient = useQueryClient();

  /**
   * Fetch whole-appointment-type bookable time suggestions for a desired duration within
   * a search window. Returns the proposal list (start/end ranges the appointment type can
  ...
  const fetchBookableSlots = async (
    ...
    const result = await appointmentTypesBookableSlotsList({
   ...
   * AppointmentTypeRangeAvailability for that range (or undefined if the backend
  ...
  const fetchSlotAvailability = async (
    ...
    startTime: string,
    endTime: string
  ): Promise<AppointmentTypeRangeAvailability | undefined> => {
  ...
  /**
   * Book the appointment type event. Sends slot_selections (which calendars satisfy which
  ...
  const bookAppointmentTypeEvent = async (
    params: BookAppointmentTypeEventParams
    ...
    const result = await appointmentTypesEventsCreate({
      path: { id: String(appointmentTypeId) },
      body: {
        title,
    ...
    if (!result.data) {
      throw new Error(
        'Unexpected empty response from appointmentTypesEventsCreate'
      );
    }
