/**
 * useAppointmentTypeScopedQuota — list, create, partial update, and delete appointment-type-scoped
 * quota rules for one slot (optionally scoped to one calendar).
 *
 * REST surface: `.../appointment-types/{appointment_type_id}/slots/{slot_id}/quota-rules/`
 * (see the handoff doc, `ai-plans/2026-08-05-CALENDAR_APPOINTMENT_TYPE_SCOPED_AVAILABILITY_HANDOFF.md`,
 * section 3). Mirrors use-appointment-type-scoped-windows.ts's shape (predicate-based
 ...
 * surface. Create returns the `AppointmentTypeScopedQuotaRule` object directly (201),
...
import {
  appointmentTypesSlotsQuotaRulesCreateMutation,
  appointmentTypesSlotsQuotaRulesListOptions,
  appointmentTypesSlotsQuotaRulesPartialUpdateMutation,
} from '@/client/@tanstack/react-query.gen';
import { appointmentTypesSlotsQuotaRulesDestroy } from '@/client';
import type {
  AppointmentTypeScopedQuotaRule,
  AppointmentTypeScopedQuotaRuleCreate,
  PatchedAppointmentTypeScopedQuotaRuleUpdate,
} from '@/client';

// The `_id` the generated factory tags every appointmentTypesSlotsQuotaRulesList*
...
export const APPOINTMENT_TYPE_SCOPED_QUOTA_LIST_OPERATION_ID =
  'appointmentTypesSlotsQuotaRulesList';

// Matches use-appointment-type-scoped-config-summary.ts's SUMMARY_PAGE_SIZE and the
...
export const APPOINTMENT_TYPE_SCOPED_QUOTA_PAGE_SIZE = 200;
...
export interface UseAppointmentTypeScopedQuotaOptions {
...
export function useAppointmentTypeScopedQuota({
  appointmentTypeId,
  slotId,
  calendarId,
  enabled = true,
}: UseAppointmentTypeScopedQuotaOptions) {
    ...
    queryClient.invalidateQueries({
      predicate: (query) =>
        ...
        (query.queryKey[0] as { _id?: string })?._id ===
          APPOINTMENT_TYPE_SCOPED_QUOTA_LIST_OPERATION_ID,
    });
  }, [queryClient]);
  ...
  const createQuotaRuleMutation = useMutation({
    ...
    // appointment-type-quota-rules.tsx renders an inline, batch-aware OverLimitAlert for
  ...
  ): Promise<AppointmentTypeScopedQuotaRule> =>
  ...
  const deleteQuotaRuleMutation = useMutation({
    mutationFn: async (
      ...
      const { response } = await appointmentTypesSlotsQuotaRulesDestroy({
        path: {
          appointment_type_id: input.appointmentTypeId,
          slot_id: input.slotId,
          id: String(input.ruleId),
      ...
      if (!response) {
        throw new Error(
          'Failed to delete appointment-type-scoped quota rule (no response)'
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to delete appointment-type-scoped quota rule (${response.status})`
        );
      }
  ...
  return {
     ...
     */
    isTruncated:
      (quotaQuery.data?.count ?? 0) > APPOINTMENT_TYPE_SCOPED_QUOTA_PAGE_SIZE,
